...
/**
 * Cryptographic helpers for the Leapmotor cloud API.
 *
 * Cross-checked against markoceri/leapmotor-api `docs/api.md`:
 *  - SHA-256 signature for the login call
 *  - HMAC-SHA256 signature for every authenticated call, built from all
 *    header + body field VALUES sorted ALPHABETICALLY BY KEY
 *  - HKDF-SHA256 for the per-session HMAC signing key
 *  - AES-128-CBC (PKCS#7) for the vehicle operation password (PIN)
 *  - a fixed-round-key SM4 routine to derive the PKCS#12 password
 *
 * ---------------------------------------------------------------------------
 * FIXES vs. the original app
 * ---------------------------------------------------------------------------
 *  1. Signature input is now generated by ONE generic routine that sorts the
 *     fields alphabetically by key. The original hard-coded a different field
 *     order per endpoint, which silently produced invalid signatures as soon as
 *     a body parameter (refreshToken, remoteCtlId, operatePassword, ...) had to
 *     be folded into the input.
 *  2. `nonce` now uses `crypto.randomInt` instead of `Math.random`.
 *  3. Added `extractDeviceIdFromToken()` - the Leapmotor gateway expects the
 *     deviceId that is embedded in the JWT it hands out at login. The original
 *     app kept its own random deviceId forever, so every signed request after
 *     login was signed with the wrong deviceId.
 *  4. `deriveSignKey()` validates its inputs instead of silently deriving a key
...
// ---------------------------------------------------------------------------
// SM4 (only used to derive the PKCS#12 account-certificate password)
// ---------------------------------------------------------------------------
...
/**
 * Derive the account PKCS#12 password from the login response fields
 * `id` (accountId) and `uid`.
 *
 *   cn        = MD5(accountId)                       (hex)
 *   appInput  = cn + cn[::2] + uid[1::2]
 *   digest    = SHA256(appInput)                     (32 raw bytes)
 *   encoded   = SM4-ECB(digest)                      (static round keys)
 *   password  = Base64(encoded[:12])[:15]
 */
...
// ---------------------------------------------------------------------------
// AES-128-CBC encryption of the vehicle operation password (PIN)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// HKDF-SHA256 signing key derivation
// ---------------------------------------------------------------------------
...
/**
 * The gateway embeds the deviceId in the JWT payload:
 *   payload.user_name === "<userId>,<something>,<deviceId>,..."
 *
 * Every signed request after login MUST use that deviceId, otherwise the
 * gateway rejects the signature (or silently returns "token invalid").
 *
 * @returns {string|null}
 */
...
/** Returns the JWT expiry (ms epoch) or null when it cannot be determined. */
...
// ---------------------------------------------------------------------------
// Header / signature building
// ---------------------------------------------------------------------------

function randomNonce() {
  // 6-7 digit numeric nonce, cryptographically random.
...
/**
 * Build the HMAC-SHA256 signature input: all field values concatenated,
 * sorted alphabetically by field name (see docs/api.md).
 */
...
/**
 * Login uses a plain SHA-256 hash with a FIXED (non alphabetical) field order.
 */
...
/**
 * Standard signed headers. Optional `vin` and any body parameters take part in
 * the signature and are merged in before the alphabetical sort.
 */
