...
function applyStoredCertificates(homey) {
  let resolved;
  try {
    resolved = resolveAppCertificates(homey);
  } catch (err) {
    return { ok: false, error: err.message };
  }
  clients.forEach((entry) => {
    if (entry.client && typeof entry.client.setAppCertificate === 'function') {
      entry.client.setAppCertificate(resolved.cert, resolved.key);
    }
  });
  return { ok: true, cert: resolved.cert, key: resolved.key };
}
