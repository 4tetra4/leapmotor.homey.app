...
function map(statusData) {
  ...
  const chargeCompleted = toBoolean(firstDefined(data, ['chargeCompleted', 'chargeComplete', 'chargeFinished']));
  const batteryFull = chargeCompleted === true || (soc !== null && soc >= FULL_CHARGE_SOC);
  if (charging === true && chargeRemain !== null && chargeRemain >= 0) {
    set('leapmotor_charge_remaining', chargeRemain);
  } else if (charging === false && batteryFull) {
    set('leapmotor_charge_remaining', 0);
  } else if (charging === false) {
    clear('leapmotor_charge_remaining');
  }
  ...
  const gearRaw = firstDefined(data, ['gearStatus', 'gear', 'gearPosition']);
  const gearNumber = toInteger(gearRaw);
  if (gearNumber !== null) set('__gearStatus', gearNumber);
  let gear = null;
  if (gearNumber === 0) gear = 'park';
  else if (gearNumber === 1) gear = 'reverse';
  else if (gearNumber === 2) gear = 'neutral';
  else if (gearNumber === 3) gear = 'drive';
  else if (gearRaw !== null && gearRaw !== undefined && gearRaw !== '') {
    const named = String(gearRaw).trim().toLowerCase();
    if (named === 'p' || named === 'park' || named === 'parking') gear = 'park';
    else if (named === 'd' || named === 'drive' || named === 'driving') gear = 'drive';
    else if (named === 'n' || named === 'neutral') gear = 'neutral';
    else if (named === 'r' || named === 'reverse') gear = 'reverse';
  }
  set('leapmotor_gear', gear);
