...
class LeapmotorApp extends Homey.App {
  async onInit() {
    ...
    this._leftOpenState = new Map();
    this._registerFlowCards();
    this.log('Leapmotor app has been initialised');
  }
  _device(args) {
    const device = args && args.device;
    if (!device) throw new Error('This Flow card is no longer linked to a Leapmotor vehicle');
    return device;
  }
  _action(id, handler) {
    try {
  ...
  _normalizeItem(val) {
    ...
    if (s === 'doors' || s === 'door') return 'door';
    if (s === 'windows' || s === 'window') return 'window';
    if (s === 'trunk' || s === 'boot' || s === 'boot_control') return 'trunk';
    if (s === 'sunshade' || s === 'sun_shade' || s === 'sunroof') return 'sunshade';
    return s;
  }
  _registerFlowCards() {
    ...
    this._condition('gear_is', (device, args) => device.getCapabilityValue('leapmotor_gear') === args.gear);
    ...
    try {
      this.homey.flow.getDeviceTriggerCard('gear_changed').registerRunListener(async (args, state) => {
        if (!args || !args.gear || args.gear === 'any') return true;
        return args.gear === state.gear;
      });
    } catch (err) {
      this.error(`Could not register gear_changed: ${err.message}`);
    }
    try {
    ...
    try {
      this.homey.flow.getDeviceTriggerCard('opening_changed').registerRunListener(async (args, state) => {
        if (state && (state.state === 'closed' || state.state === 'close')) {
          const norm = this._normalizeItem(state.item);
          for (const k of Array.from(this._leftOpenState.keys())) {
            if (k.includes(`:${norm}:`) || k.includes(`:${state.item}:`)) {
              this._leftOpenState.set(k, false);
            }
          }
        }
        if (!args || !args.item || !args.state) return true;
        const itemMatch = args.item === 'any' || this._normalizeItem(args.item) === this._normalizeItem(state && state.item);
        const stateMatch = args.state === (state && state.state);
        return itemMatch && stateMatch;
      });
    } catch (err) {
    ...
    try {
      this.homey.flow.getDeviceTriggerCard('opening_left_open').registerRunListener(async (args, state) => {
        if (!args || !state) return false;
        if (args.item && args.item !== 'any') {
          const argNorm = this._normalizeItem(args.item);
          const stateNorm = this._normalizeItem(state.item);
          if (argNorm !== stateNorm) return false;
        }
        const rawThreshold = args.minutes !== undefined
          ? args.minutes
          : (args.duration !== undefined
            ? args.duration
            : (args.time !== undefined ? args.time : args.open_minutes));
        const threshold = Number(rawThreshold);
        const openMinutes = Number(state.minutes !== undefined ? state.minutes : state.open_minutes);
        if (!Number.isFinite(threshold) || !Number.isFinite(openMinutes)) return false;
        const wanted = threshold > 0 ? threshold : 1;
        const due = openMinutes >= wanted;
        const devId = this._deviceId(args);
        const normItem = this._normalizeItem(state.item);
        const key = `${devId}:${normItem}:${wanted}`;
        if (openMinutes === 0) {
          this._leftOpenState.set(key, false);
          return false;
        }
        const last = this._leftOpenState.get(key);
        this._leftOpenState.set(key, due);
        return last !== true && due === true;
      });
    } catch (err) {
      this.error(`Could not register opening_left_open: ${err.message}`);
    }
